...
def cpt_detail(request,cpt_id):
    ...
    nb_ope_vielles=Ope.objects.filter(compte__pk=cpt_id).order_by('-date').filter(date__lte=date_limite).filter(is_mere=False).filter(pointe__in=[u'na',u'p']).count()
...
def ope_creation(request, cpt_id):
    ...
    ope=Ope(compte=cpt, date=datetime.date.today(), montant=0, devise=devise, tiers = None, cat = None, scat= None, Notes = None, moyen = None, numcheque="", )
    pass
