...
def export(request):
    ...
    ("%s" % (i - 1) for i in Compte.objects.all().values_list('id', flat=True))
...
for co in Compte.objects.all().order_by('id'):
    ...
    et.SubElement(xml_detail, "Ordre_du_tri").text = '/'.join(
        [str(o.grisbi_id) for o in Moyen.objects.filter(compte=co.id).order_by('id')])
    ##types:
    ...
    for ope in Ope.objects.filter(compte=co.id).order_by('date'):
        ...
        xml_element.set('Rdc',
                        '0') #NOT IN BDD mais pour l'instant ce n'est pas géré par les devises de plus , je ne vois pas vraiment l'utilite
...
for c in Cat.objects.all().order_by('id'):
    ...
    xml_cate.set('No_derniere_sous_cagegorie',
                 Format.max(Scat.objects.filter(cat=c.id), defaut='0', champ='grisbi_id'))
...
for c in Ib.objects.all().order_by('id'):
    if c is None:
        ...
        xml_ib.set('No_derniere_sous_imputation',
                   Format.max(Sib.objects.filter(ib=c.id), defaut='0', champ='grisbi_id'))
