...
class Export_xml(generic.View):
    ...
    def get(self, *args, **kwargs):
        ...
        query = models.Cours.objects.filter(lastupdate__gte="2010-01-01").select_related('Compte', 'Moyen', 'Rapp', 'Tiers')
        ...
        if not self.debug:
            ...
            reponse["Content-Disposition"] = "attachment; filename=%s_%s.%s" % (
                self.nomfich, time.strftime("%d_%m_%Y_%h_%m_%s", time.localtime()), self.extension_file)
        else:
            return reponse
