    ...
    def tableau(self, fich, moyen_virement):
        ...
        for row in fich:
                ...
                for attr in liste_colonnes:
                    try:
                        if getattr(row,attr,"colonne inexistante") == "colonne inexistante":
                            colonnes_oublies.append(attr)#pragma nocover
            ...
            if row.jumelle:
                ...
                if origine and dest:
                    if dest == origine:
                        self.erreur.append(u'attention, virement impossible entre le meme compte à la ligne %s' % row.ligne)
                        continue
                    if self.comptes.goc(dest) != ope['compte_id'] and self.comptes.goc(origine) != ope['compte_id']:
                        self.erreur.append(u"le compte designe doit etre un des deux comptes 'tiers' ligne %s"  % row.ligne)
                        continue
                    ...
                    if self.comptes.goc(dest) == ope['compte_id'] and row.montant < 0:
                        ...
                        self.erreur.append(u"il faudrait swap %s et %s a la ligne %s" % (origine, dest, row.ligne))
                        continue
            ...
            if row.ope_titre:
                ...
                if 'titre_ ' in row.tiers:
                    ...
                    self.erreur.append(u"ce tiers '%s' ne peut etre un titre a la ligne %s" % (row.tiers,row.line_num))
                    continue
            else:
                ope['ope_titre'] = False
